...
def main():
    ...
    # THE EMISSION IS NOT ALWAYS HERE. reference/rooms/ is checked in; the
    # zone emission is not. A machine that only needs the derived TS rewritten
    # (a new field on DropRow, say) rebuilds from the room files it already
    # has, and skips rewriting them.
    from_rooms = not rooms
    if from_rooms:
        for path in sorted(glob.glob(os.path.join(OUT, "*.json"))):
            ...
            rooms[data["room"]] = {
                ...
                "name": data["name"],
                "band": data["band"],
                "items": {i["itemId"]: i for i in data["items"]},
                "worldDrops": data["worldDrops"],
            }
    ...
    # THE RAIL'S INDEX. Six numbers per item — id, the level it becomes
    # available, a bitmask of the classes it appears for, the slot it is
    # worn in, the quality rank, and the item level — in byRank order,
    ...
    #
    # THE ITEM LEVEL IS THE JUDGE'S HALF. lib/upgrade.ts prices a row against
    # the worn piece by quality and item level; without the sixth number every
    # counting surface would call a green over your epic an "upgrade", which
    # is the 2026-09-02 bug this field exists to kill.
    ...
    with open(DROPS_TS, "w") as f:
        ...
        for q in sorted(quality_rank, key=quality_rank.get):
            f.write(f'  "{q}",\n')
        f.write("] as const;\n\n")
        f.write("/** [itemId, availableAtLevel, classMask, slot, quality, itemLevel], best first. */\n")
        f.write("export type DropRow = [number, number, number, number, number, number];\n\n")
        ...
        for rid in sorted(rooms):
            ...
            for i in items:
                ...
                rows.append(f'[{i["itemId"]},{i["availableAtLevel"]},{mask},{slot},{quality_rank[i["quality"]]},{i["itemLevel"]}]')
