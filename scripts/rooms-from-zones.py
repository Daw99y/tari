...
def main():
    ...
    # THE RAIL'S INDEX. Three numbers per item — id, the level it becomes
    # available, and a bitmask of the classes it appears for — in byRank order,
    ...
    with open(DROPS_TS, "w") as f:
        f.write("// GENERATED by scripts/rooms-from-zones.py — do not edit.\n")
        f.write("//\n")
        f.write("// Every drop the rail needs to count, and nothing else. See the block\n")
        f.write("// above this file's writer in scripts/rooms-from-zones.py for why it is\n")
        f.write("// three numbers, and why the order it is in is load-bearing.\n\n")
        f.write("/** The class order the bitmask is written in. */\n")
        f.write("export const DROP_CLASSES = [\n")
        for c in classes_in_order:
            f.write(f'  "{c}",\n')
        f.write("] as const;\n\n")
        f.write("/** [itemId, availableAtLevel, classMask], best first. */\n")
        f.write("export type DropRow = [number, number, number];\n\n")
        ...
        for rid in sorted(rooms):
            ...
            for i in items:
                ...
                rows.append(f'[{i["itemId"]},{i["availableAtLevel"]},{mask}]')
